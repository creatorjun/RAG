...
1. 사내 공식 분류체계와 문서 태그로 1차 주제를 결정한다.
2. 미분류 문서에만 임베딩 계층 군집을 보조적으로 사용한다.
3. Map 단계에서 청크를 직접 요약하지 않고 승인된 주장과 인용을 주제별 근거 카드로 변환한다.
4. Reduce 단계에서 근거 카드를 병합하며 중복, 충돌, 버전 범위를 유지한다.
5. 최상위 단계에서 문서 목차와 교차 링크만 구성한다.
6. 모든 사실 문장에 내부 `source_span` 또는 외부 URL 근거를 연결한다.
7. 근거 없는 문장을 자동 검사해 게시를 차단한다.
